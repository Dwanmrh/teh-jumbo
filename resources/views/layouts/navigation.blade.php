    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[64px] sm:h-[72px] flex justify-between items-center">
        ...
        <div class="flex items-center gap-4">
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3 group select-none">
                <img src="{{ asset('assets/images/logo-teh.png') }}"
                    class="h-8 w-auto sm:h-9 object-contain transform group-hover:-rotate-12 transition-transform duration-500"
                    alt="Logo">
                <div class="flex flex-col justify-center">
                    <span class="text-lg sm:text-xl font-extrabold text-stone-800 tracking-tight leading-none">Teh Solo
                        <span class="text-brand-600">Jumbo</span></span>
                    <span
                        class="text-[9px] sm:text-[10px] font-bold text-stone-400 tracking-[0.15em] uppercase mt-0.5 group-hover:text-brand-500 transition-colors">
        ...
        <div class="flex items-center">
            <x-dropdown align="right" width="48">
                <x-slot name="trigger">
                    <button
                        class="flex items-center gap-2 py-1 pl-2 pr-1 sm:pr-2 rounded-full hover:bg-stone-100 border border-transparent hover:border-stone-200 transition-all duration-300 group">
                        <div class="hidden sm:block text-right leading-tight mr-1.5">
                            <div class="text-xs font-bold text-stone-800 group-hover:text-brand-700 transition-colors">
                                {{ Str::limit($user->name, 15) }}</div>
                            <div class="text-[10px] text-stone-400 font-medium capitalize tracking-wide">
                                {{ $user->role }}</div>
                        </div>
                        <div
                            class="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-stone-900 text-white flex items-center justify-center text-xs font-bold border-[3px] border-stone-100 shadow-sm group-hover:scale-105 transition-transform">
...
<div class="hidden md:block w-full sticky top-[64px] sm:top-[72px] z-40 pointer-events-none mb-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pointer-events-auto mt-6">
        <div class="relative group mx-auto max-w-fit">
            <div
                class="bg-white/80 backdrop-blur-md px-2 py-2 rounded-full flex items-center shadow-soft border border-stone-200/80 ring-1 ring-stone-900/5 gap-1 overflow-x-auto no-scrollbar max-w-full">
                @foreach($allLinks as $link)
                            @php $isActive = request()->routeIs($link['route']) || request()->routeIs(explode('.', $link['route'])[0] . '.*'); @endphp
                            <a href="{{ route($link['route']) }}" class="flex items-center gap-2 px-4 lg:px-5 py-2 rounded-full transition-all duration-300 group relative select-none whitespace-nowrap
                                          {{ $isActive
                    ...
                    : 'text-stone-500 hover:text-brand-600 hover:bg-brand-50 hover:scale-[1.02]'
                                ...
                                <span
                                    class="material-symbols-rounded text-[20px] {{ $isActive ? 'filled text-brand-400' : '' }}">{{ $link['icon'] }}</span>
