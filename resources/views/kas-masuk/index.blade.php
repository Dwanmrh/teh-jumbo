<x-app-layout>
    ...
    {{-- Container Utama --}}
    <div class="min-h-screen bg-stone-50/50 pb-36" x-data="{ showDetail: false, selectedItem: {} }">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
                <div>
                    ...
                    <p class="text-stone-500 text-xs md:text-sm mt-1.5 max-w-lg leading-relaxed">Kelola dan pantau
                        seluruh arus pemasukan dana.</p>
                </div>
            ...
            <div class="mb-6">
                <div
                    class="relative bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-[2rem] p-6 md:p-10 shadow-2xl shadow-emerald-900/20 overflow-hidden group">
                    <div
                        class="absolute top-0 right-0 -mt-10 -mr-10 w-48 h-48 md:w-64 md:h-64 bg-white/10 rounded-full blur-3xl group-hover:bg-white/20 transition-all duration-700">
                    </div>
                    <div
                        class="absolute bottom-0 left-0 -mb-10 -ml-10 w-32 h-32 md:w-40 md:h-40 bg-black/10 rounded-full blur-2xl">
                    </div>

                    <div
                        class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6">
                        <div>
                            <div class="flex items-center gap-2 mb-2 md:mb-3">
                                <span
                                    class="bg-emerald-500/50 border border-emerald-400/30 text-emerald-50 px-2.5 py-0.5 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-wider">Total
                                    Masuk</span>
                            </div>
                            <h2
                                class="text-3xl md:text-6xl font-black text-white tracking-tighter drop-shadow-sm flex items-start">
                                <span class="text-emerald-200 text-lg md:text-4xl font-bold mr-1 mt-1 md:mt-2">Rp</span>
                                {{ number_format($totalKas, 0, ',', '.') }}
                            </h2>
                            <p
                                class="text-emerald-100 mt-2 md:mt-3 font-medium flex items-center gap-2 text-xs md:text-base opacity-90">
                                <span class="flex h-1.5 w-1.5 md:h-2 md:w-2 relative">
                                    ...
                                    <span
                                        class="relative inline-flex rounded-full h-1.5 w-1.5 md:h-2 md:w-2 bg-white"></span>
                                </span>
                                {{ $jumlahTransaksi }} Transaksi tercatat
                            </p>
                        </div>
                        <div
                            class="hidden md:block bg-white/10 p-4 rounded-3xl backdrop-blur-sm border border-white/10 shadow-inner">
            ...
            <form method="GET" action="{{ route('kas-masuk.index') }}" id="filterForm"
                ...
                <div class="flex flex-col md:flex-row gap-2 md:gap-3">
                    <div class="relative flex-1 w-full group">
                        <span
                            class="material-symbols-rounded absolute left-4 top-1/2 -translate-y-1/2 text-stone-400 text-xl group-focus-within:text-emerald-500 transition-colors">search</span>
                    ...
                    <div class="grid grid-cols-5 gap-2 w-full md:w-auto">
                        ...
                        <div class="relative col-span-2" x-data="{ open: false }" @click.outside="open = false">
                            <input type="hidden" name="filter_harga" id="hargaSelect"
                                value="{{ request('filter_harga') }}">
                            <button type="button" @click="open = !open"
                                ...
                                <span class="truncate"
                                    x-text="$el.previousElementSibling.value == '0-50000' ? '0-50rb' : ($el.previousElementSibling.value == '51000-500000' ? '51rb-500rb' : ($el.previousElementSibling.value == '500001-999999999' ? '> 500rb' : 'Harga'))">Harga</span>
                                <div x-show="$el.previousElementSibling.value" style="display: none;"
                                    class="absolute top-1 right-1 w-2 h-2 bg-emerald-500 rounded-full"></div>
                            </button>
                            ...
                            <div x-show="open" style="display: none;"
                                ...
                                <button type="button" onclick="setFilter('hargaSelect', '')" @click="open = false"
                                    class="text-left px-3 py-2 rounded-xl text-xs font-medium hover:bg-emerald-50 hover:text-emerald-700 transition-colors">Semua</button>
                                <button type="button" onclick="setFilter('hargaSelect', '0-50000')"
                                    @click="open = false"
                                    class="text-left px-3 py-2 rounded-xl text-xs font-medium hover:bg-emerald-50 hover:text-emerald-700 transition-colors">0
                                    - 50rb</button>
                                <button type="button" onclick="setFilter('hargaSelect', '51000-500000')"
                                    @click="open = false"
                                    class="text-left px-3 py-2 rounded-xl text-xs font-medium hover:bg-emerald-50 hover:text-emerald-700 transition-colors">51rb
                                    - 500rb</button>
                                <button type="button" onclick="setFilter('hargaSelect', '500001-999999999')"
                                    @click="open = false"
                                    class="text-left px-3 py-2 rounded-xl text-xs font-medium hover:bg-emerald-50 hover:text-emerald-700 transition-colors">>
                                    500rb</button>
                            </div>
                        </div>
                        ...
                        <div class="relative col-span-2"
                            x-data="{ open: false, isCustom: '{{ request('filter_waktu') }}' == 'custom' }"
                            @click.outside="open = false">
                            <input type="hidden" name="filter_waktu" id="tanggalSelect"
                                value="{{ request('filter_waktu') }}">
                            <button type="button" @click="open = !open"
                                class="w-full h-full px-2 py-2.5 md:px-4 md:py-0 rounded-2xl border border-stone-200 bg-white hover:bg-stone-50 active:bg-stone-100 text-stone-600 text-xs md:text-sm font-bold flex items-center justify-center gap-1.5 transition-colors relative">
                                <span class="material-symbols-rounded text-base text-emerald-500">calendar_today</span>
                                <span class="truncate"
                                    x-text="$el.previousElementSibling.value == 'hari-ini' ? 'Hari Ini' : ($el.previousElementSibling.value == 'minggu-ini' ? 'Minggu Ini' : ($el.previousElementSibling.value == 'bulan-ini' ? 'Bulan Ini' : ($el.previousElementSibling.value == 'custom' ? 'Custom' : 'Waktu')))">Waktu</span>
                                <div x-show="$el.previousElementSibling.value" style="display: none;"
                                    class="absolute top-1 right-1 w-2 h-2 bg-emerald-500 rounded-full"></div>
                            </button>
                            ...
                            <div x-show="open" style="display: none;"
                                ...
                                <button type="button" onclick="setFilter('tanggalSelect', '', false)"
                                    @click="open = false; isCustom = false"
                                    class="text-left px-3 py-2 rounded-xl text-xs font-medium hover:bg-emerald-50 hover:text-emerald-700 transition-colors">Semua</button>
                                <button type="button" onclick="setFilter('tanggalSelect', 'hari-ini', false)"
                                    @click="open = false; isCustom = false"
                                    class="text-left px-3 py-2 rounded-xl text-xs font-medium hover:bg-emerald-50 hover:text-emerald-700 transition-colors">Hari
                                    Ini</button>
                                <button type="button" onclick="setFilter('tanggalSelect', 'minggu-ini', false)"
                                    @click="open = false; isCustom = false"
                                    class="text-left px-3 py-2 rounded-xl text-xs font-medium hover:bg-emerald-50 hover:text-emerald-700 transition-colors">Minggu
                                    Ini</button>
                                <button type="button" onclick="setFilter('tanggalSelect', 'bulan-ini', false)"
                                    @click="open = false; isCustom = false"
                                    class="text-left px-3 py-2 rounded-xl text-xs font-medium hover:bg-emerald-50 hover:text-emerald-700 transition-colors">Bulan
                                    Ini</button>
                                <button type="button" onclick="setFilter('tanggalSelect', 'custom', true)"
                                    @click="isCustom = true"
                                    class="text-left px-3 py-2 rounded-xl text-xs font-medium hover:bg-emerald-50 hover:text-emerald-700 transition-colors flex justify-between items-center">
                                    <span>Pilih Tanggal (Custom)</span> <span class="material-symbols-rounded text-sm"
                                        x-show="isCustom" style="display: none;">check</span>
                                </button>
                                <div x-show="isCustom" style="display: none;"
                                    class="p-2 bg-stone-50 rounded-xl mt-1 border border-stone-100 space-y-2">
                                    <input type="date" name="start_date" id="startDateInput"
                                        value="{{ request('start_date') }}" onchange="applyAllFilters()"
                                        class="w-full bg-white border-stone-200 rounded-lg text-xs py-1.5 px-2">
                                    <input type="date" name="end_date" id="endDateInput"
                                        value="{{ request('end_date') }}" onchange="applyAllFilters()"
                                        class="w-full bg-white border-stone-200 rounded-lg text-xs py-1.5 px-2">
                                </div>
                            </div>
                        </div>
                        ...
                        <a href="{{ route('kas-masuk.index') }}"
                            class="col-span-1 h-full flex items-center justify-center rounded-2xl border border-stone-200 hover:bg-emerald-50 hover:text-emerald-600 text-stone-500 transition-colors bg-stone-50">
            ...
            @if(Auth::user()->role === 'admin')
                <div class="mb-4">
                    ...
                    <button id="bulkDeleteBtn" onclick="submitBulkDelete()" 
                        class="hidden bg-rose-600 text-white px-6 py-3 rounded-2xl items-center gap-2 transition-all shadow-lg flex"
                    >
                        ...
                        <span class="font-bold text-sm tracking-wide">
                            Hapus Item (<span id="selectedCount">0</span>)
                        </span>
                    </button>
            ...
            <div id="kasDataContainer"
                class="hidden md:block bg-white rounded-[2rem] shadow-sm border border-stone-200 overflow-hidden">
                            ...
                            <tr>
                                <th class="p-6 text-xs font-bold uppercase tracking-wider w-10">
                                    <input type="checkbox" id="selectAll"
                                        class="w-4 h-4 text-emerald-600 bg-stone-100 border-stone-300 rounded focus:ring-emerald-500">
                                </th>
                                ...
                                <tr class="hover:bg-stone-50 transition-colors group filter-item"
                                    ...
                                    <td class="p-6 align-top">
                                        <input type="checkbox" name="ids[]" value="{{ $item->id }}"
                                            class="user-checkbox w-4 h-4 text-emerald-600 bg-stone-100 border-stone-300 rounded focus:ring-emerald-500">
                                    </td>
                                    <td class="p-6 align-top">
                                    ...
                                    <td class="p-6 align-top">
                                        <div class="font-medium text-stone-700 line-clamp-2 max-w-[250px] leading-relaxed">
                                            {{ $item->keterangan }}
                                        </div>
                                    ...
                                    <td class="p-6 align-top text-center">
                                        ...
                                        @endphp
                                        <span
                                            class="inline-block px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wide border shadow-sm {{ $badgeClass }}">
                                            {{ $item->kategori }}
                                        </span>
                                    </td>
                                    <td class="p-6 align-top text-right whitespace-nowrap">
                                        <span class="font-bold text-emerald-600 text-base">Rp
                                            {{ number_format($item->total, 0, ',', '.') }}</span>
                                    </td>
                                    <td class="p-6 align-top text-center">
                                        <span
                                            class="text-xs font-semibold text-stone-500 bg-stone-100 px-3 py-1.5 rounded-full border border-stone-200">
                                        ...
                                        @if(Auth::user()->role === 'admin')
                                            <div
                                                ...
                                                <a href="{{ route('kas-masuk.edit', $item->id) }}"
                                                    class="p-2 rounded-lg text-stone-500 hover:bg-emerald-50 hover:text-emerald-600 transition-colors">
                                                    <span class="material-symbols-rounded">edit</span>
                                                </a>
                                                <button type="button"
                                                    class="delete-btn p-2 rounded-lg text-stone-500 hover:bg-rose-50 hover:text-rose-600 transition-colors"
                                                    data-id="{{ $item->id }}" data-deskripsi="{{ $item->keterangan }}">
                                                    <span class="material-symbols-rounded">delete</span>
                                                </button>
                                                <form action="{{ route('kas-masuk.destroy', $item->id) }}" method="POST"
                                                    id="deleteForm-{{ $item->id }}" class="hidden">
                                                    @csrf @method('DELETE')
                                                </form>
                                            </div>
                                        @else
                                ...
                                <tr id="noDataRow">
                                    <td colspan="7" class="py-20 text-center">
                                        <div class="flex flex-col items-center justify-center text-stone-300">
                                            <span
                                                class="material-symbols-rounded text-6xl mb-4 bg-stone-50 rounded-full p-6">receipt_long</span>
                                        ...
                                        <div class="flex justify-between items-start mb-2">
                                            <div class="w-2/3">
                                                <div class="flex items-center gap-1.5 mb-1">
                                                    <span
                                                        class="inline-block px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wide border shadow-sm {{ $badgeClass }}">
                                                        {{ $item->kategori }}
                                                    </span>
                                                    <span
                                                        class="text-[9px] text-stone-400">{{ \Carbon\Carbon::parse($item->tanggal_transaksi)->format('d M') }}</span>
                                                </div>
                                                <h3 class="font-bold text-stone-800 text-sm line-clamp-1 leading-tight">
                                                    {{ $item->keterangan }}
                                                </h3>
                                            </div>
                                            <div class="text-right flex-1">
                                                <p class="font-extrabold text-emerald-600 text-base">Rp
                                                    {{ number_format($item->total, 0, ',', '.') }}
                                                </p>
                                            </div>
                                        </div>
                                        <div class="flex justify-between items-center pt-2 border-t border-stone-50">
                                            <p class="text-[10px] text-stone-500 line-clamp-1 italic w-3/4">{{ $item->jumlah }} x Rp
            ...
            <div x-show="showDetail" class="fixed inset-0 z-[100] md:hidden flex flex-col bg-white"
                ...
                <div
                    class="px-5 py-3 border-b border-stone-100 flex items-center justify-between bg-white sticky top-0 z-10">
                    <button @click="showDetail = false"
                        class="w-8 h-8 rounded-full bg-stone-50 flex items-center justify-center text-stone-600 active:bg-stone-200">
                ...
                <div class="flex-1 overflow-y-auto p-5 bg-stone-50/50">
                    <div class="text-center mb-6 mt-2">
                        <span
                            class="inline-block px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-2 shadow-sm border"
                            :class="{
                        ...
                        <h3 class="text-3xl font-black text-stone-800 tracking-tight"
                            x-text="'Rp ' + selectedItem.nominal"></h3>
                    ...
                    <div class="bg-white rounded-3xl p-5 shadow-sm border border-stone-100 space-y-4">
                        <div class="grid grid-cols-2 gap-3 pb-3 border-b border-stone-50">
                            ...
                            <div>
                                ...
                                <span class="font-bold text-stone-700 text-sm"
                                    x-text="selectedItem.metode_pembayaran"></span>
                            </div>
                        </div>
                        <div>
                            <span class="text-[10px] text-stone-400 block mb-0.5">Keterangan</span>
                            <p class="font-medium text-stone-800 text-sm leading-relaxed"
                                x-text="selectedItem.keterangan || '-'"></p>
                        </div>
                        <div class="bg-stone-50 p-3 rounded-xl border border-stone-100">
                            ...
                            <span class="font-bold text-stone-600 text-sm"
                                x-text="selectedItem.jumlah + ' x Rp ' + selectedItem.harga_satuan"></span>
                        </div>
                    </div>
                </div>
                ...
                @if(Auth::user()->role === 'admin')
                    <div class="p-5 bg-white border-t border-stone-100 flex gap-3 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
                        <a :href="selectedItem.edit_url"
                            class="flex-1 bg-white border border-stone-200 text-stone-700 font-bold py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-stone-50 active:scale-95 transition-all text-sm">
                        ...
                        <button @click="confirmDelete(selectedItem.id, selectedItem.keterangan, selectedItem.delete_url)"
                            class="flex-1 bg-rose-600 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-rose-500/30 hover:bg-rose-700 active:scale-95 transition-all text-sm">
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                function updateBulkDeleteState() {
                    ...
                    if (count > 0) {
                        // Tampilkan tombol
                        bulkDeleteBtn.classList.remove('hidden', 'opacity-50', 'cursor-not-allowed');
